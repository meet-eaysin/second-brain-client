...
export function DocumentListView({
    ...
    const titleProperty = properties.find(p => p.name.toLowerCase() === 'title') || 
                         properties.find(p => p.type === 'text');
    const statusProperty = properties.find(p => p.name.toLowerCase().includes('status'));
    const priorityProperty = properties.find(p => p.name.toLowerCase().includes('priority'));

    // Render a single record row
    const renderRecordRow = (record: DatabaseRecord, index: number) => {
        const title = titleProperty ? record.properties?.[titleProperty.id] : record.id;
        const status = statusProperty ? record.properties?.[statusProperty.id] : null;
        const priority = priorityProperty ? record.properties?.[priorityProperty.id] : null;
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                        ...
                        {priority && (
                            <Badge 
                                variant={
                                    String(priority).toLowerCase() === 'high' ? 'destructive' :
                                    String(priority).toLowerCase() === 'medium' ? 'default' :
                                    'secondary'
                                }
                                className="text-xs"
                            >
                                ...
                                .map(property => {
                                    ...
                                    return (
                                        <div key={property.id} className="text-xs text-muted-foreground">
                                            <span className="font-medium">{property.name}:</span> {String(value)}
                                        </div>
                                    );
                                })}
                        </div>
                        ...
                        {(config.canEdit || config.canDelete) && (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    {config.canEdit && (
                                        <DropdownMenuItem onClick={(e) => handleRecordEdit(e, record)}>
                                            Edit
                                        </DropdownMenuItem>
                                    )}
                                    {config.canDelete && (
