...
export function DocumentTimelineView({
    ...
    const descriptionProperty = properties.find(p => p.name.toLowerCase().includes('description')) ||
    ...
    const sortedRecords = React.useMemo(() => {
        ...
        return [...records].sort((a, b) => {
            const dateA = a.properties?.[dateProperty.id] ? new Date(String(a.properties[dateProperty.id])) : new Date(0);
            const dateB = b.properties?.[dateProperty.id] ? new Date(String(b.properties[dateProperty.id])) : new Date(0);
    ...
    const renderRecord = (record: DatabaseRecord, isLast: boolean) => {
        ...
        const description = descriptionProperty ? record.properties?.[descriptionProperty.id] : null;
