...
import { booksDocumentViewService, type BooksViewConfig, type BookRecord } from '../services/books-document-view.service';
...
export const BOOKS_DOCUMENT_VIEW_QUERY_KEYS = {
    ...
    defaultProperties: () => ['books-document-view', 'default-properties'] as const,
    ...
    books: (filters?: any, sorts?: any) => ['books-document-view', 'books', { filters, sorts }] as const,
...
export function useBooksQuery(filters?: any, sorts?: any, pagination?: any, viewId?: string) {
    return useQuery({
        queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.books(filters, sorts),
        queryFn: () => booksDocumentViewService.getBooks(filters, sorts, pagination, viewId),
...
export function useBooksWithFiltersAndSortsQuery(options: {
    ...
    return useQuery({
        ...
        queryFn: () => booksDocumentViewService.getBooksWithFiltersAndSorts(queryOptions),
        enabled,
        staleTime: 2 * 60 * 1000, // 2 minutes
        gcTime: 10 * 60 * 1000, // 10 minutes
    });
...
export function useBooksWithViewQuery(viewId: string, additionalFilters?: Record<string, any>) {
    return useQuery({
        queryKey: ['books-document-view', 'books-with-view', viewId, additionalFilters],
        queryFn: () => booksDocumentViewService.getBooksWithFiltersAndSorts({
            viewId,
...
export function useBooksWithFiltersQuery(options: {
    status?: string[];
    genre?: string[];
    author?: string;
    tags?: string[];
    ...
    return useQuery({
        ...
        queryFn: () => booksDocumentViewService.getBooksWithFiltersAndSorts(options),
        staleTime: 2 * 60 * 1000,
        gcTime: 10 * 60 * 1000,
    });
}
...
export function useBooksByStatusQuery(status: string) {
    return useQuery({
        ...
        queryFn: () => booksDocumentViewService.getBooksWithFiltersAndSorts({ status: [status] }),
        enabled: !!status,
        staleTime: 2 * 60 * 1000,
        gcTime: 10 * 60 * 1000,
    });
...
export function useBooksByGenreQuery(genre: string) {
    return useQuery({
        ...
        queryFn: () => booksDocumentViewService.getBooksWithFiltersAndSorts({ genre: [genre] }),
        enabled: !!genre,
        staleTime: 2 * 60 * 1000,
        gcTime: 10 * 60 * 1000,
    });
...
export function useFavoriteBooksQuery() {
    return useQuery({
        ...
        queryFn: () => booksDocumentViewService.getBooksWithFiltersAndSorts({ favorite: true }),
        staleTime: 2 * 60 * 1000,
        gcTime: 10 * 60 * 1000,
    });
}
...
export function useCreateBooksViewMutation() {
    ...
    return useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.views() });
        },
    });
}
...
export function useUpdateBooksViewMutation() {
    ...
    return useMutation({
        ...
        onSuccess: (_, { viewId }) => {
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.view(viewId) });
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.views() });
        },
    });
}
...
export function useDeleteBooksViewMutation() {
    ...
    return useMutation({
        mutationFn: (viewId: string) => booksDocumentViewService.deleteBooksView(viewId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.views() });
        },
    });
...
export function useUpdateBookMutation() {
    ...
    return useMutation({
        mutationFn: ({ bookId, updates }: { bookId: string; updates: Partial<BookRecord> }) =>
            booksDocumentViewService.updateBook(bookId, updates),
        onSuccess: (updatedBook, { bookId }) => {
            console.log('✅ Book update successful:', { bookId, updatedBook });

            // Invalidate the specific book query
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.book(bookId) });

            // Invalidate ALL books-related queries with comprehensive patterns
            queryClient.invalidateQueries({
            ...
            console.log('✅ All books queries invalidated and refetched');
        },
        onError: (error) => {
...
export function useDeleteBookMutation() {
    ...
    return useMutation({
        ...
        onSuccess: (_, bookId) => {
            ...
            queryClient.removeQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.book(bookId) });
...
export function useAddBooksPropertyMutation() {
    ...
    return useMutation({
        ...
        onSuccess: (updatedView, { viewId }) => {
            ...
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.views() });
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.view(viewId) });
            queryClient.invalidateQueries({ queryKey: BOOKS_DOCUMENT_VIEW_QUERY_KEYS.defaultProperties() });

            console.log('✅ All books queries invalidated - property should now appear in table');
        },
        onError: (error) => {
